...
var addedHead = false
var score: int = 0
@onready var game_timer: Timer = get_node("/root/Node3D/GameTimer")
@onready var score_label: Label = get_node("/root/Node3D/CanvasLayer/ScoreLabel")
@onready var timer_label: Label = get_node("/root/Node3D/CanvasLayer/TimerLabel")
...
func _ready():
	...
	if not has_node("GameTimer"):
		print("Error: GameTimer node is missing!")
	else:
	...
	if not has_node("CanvasLayer/TimerLabel"):
		print("Error: TimerLabel node is missing!")
	else:
		timer_label = $CanvasLayer/TimerLabel  

	if not has_node("CanvasLayer/ScoreLabel"):
		print("Error: ScoreLabel node is missing!")
	else:
...
func _on_game_timer_timeout():
	...
	if time_left <= 0:
		timer_label.text = "Time's up! Try Again!"
		timer_label.add_theme_font_size_override("font_size", 50)
		timer_label.add_theme_color_override("font_color", Color.RED)
		timer_label.position = Vector2(300, 250)

		score_label.text = "Final Score: " + str(Global.coins)
		Global.coins = 0
...
func update_timer():
	if timer_label:
		...
		print("Warning: timer_label is null!")
